...
export class PromptComposer {
  ...
  private buildBudgetReminder(budget: BudgetContext): string {
    ...
    if (budget.utilization > 0.85) {
      lines.push(`锟?Context budget: ${(budget.utilization * 100).toFixed(0)}% used. Be concise. Avoid large tool outputs.`);
    }
    if (stepsLeft <= 3) {
      lines.push(`锟?Steps remaining: ${stepsLeft}/${budget.maxSteps}. Wrap up or produce a final answer soon.`);
    }
