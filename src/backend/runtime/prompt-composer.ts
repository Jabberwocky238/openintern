...
import type { GroupWithRoles } from '../repository/postgres/group-repository.js';
...
export class PromptComposer {
  ...
  private buildBudgetReminder(budget: BudgetContext): string {
    ...
    if (budget.utilization > 0.85) {
      lines.push(`âš?Context budget: ${(budget.utilization * 100).toFixed(0)}% used. Be concise. Avoid large tool outputs.`);
    }
    if (stepsLeft <= 3) {
      lines.push(`âš?Steps remaining: ${stepsLeft}/${budget.maxSteps}. Wrap up or produce a final answer soon.`);
    }
